...
bool TileMap::Solve_AStar(Enemy* enemy, int TileID)
{
	...
	Vector2f EndPoint = GetTilePosByID(TileID);
	...
	enemy->mapTileIDToMoveVector = tilesIDPath;
	//cout << enemy->mapTileIDToMove.size() << endl;

	return true;
}

Vector2f TileMap::GetTilePosByID(int id)
{
	//Vector2f tilePos;
	//tilePos.y = int(id / MAP_SIZE);
	//tilePos.x = id % MAP_SIZE;

	//tilePos = { tilePos.x * TILE_SIZE + TILE_SIZE / 2, tilePos.y * TILE_SIZE + TILE_SIZE / 2 };
	return allTiles[id].tile.getPosition();
}
