...
void Server::addNewClient()
{
	...
	if (server.accept(*newClient) == Socket::Done)
	{
		...
		clientsID++;
		cout << "Pozycja: " << players[newClientID].getPos().x << " " << players[newClientID].getPos().y << endl;
		...
		//p.clear();
		//p << PacketTypes::ListOfCliends;
		//p << static_cast<Uint32>(newListOfCliends->size());
		//for (list<int>::const_iterator it = newListOfCliends->begin(); it != newListOfCliends->end(); ++it)
		//	p << *it;
		//sendPackets(&p, newClientID, SendPacketsType::OnlyOne); // wyslij nowemu clientowi liste innych clientow
		...
		//p.clear();
		//p << PacketTypes::SetPlayerPosition << 1;
		//sendPacketsUDP(&p, newClientID, SendPacketsType::All);
		//p << PacketTypes::CreateEnemyAfterConnect << enemies.size();
		//for (auto& e : enemies)
		//{
		//	p << e.GetID() << e.GetPos().x << e.GetPos().y;
		//}
		//sendPackets(&p, newClientID, SendPacketsType::OnlyOne);
	}
	else
	...
	for (int i = 0; i < MAX_PLAYERS; i++)
	{
		if (clients[i] != nullptr)
		{
			...
			case Socket::Status::Done:
			{
				...
				break;
				case PacketTypes::Keyboard_Space_Clicked:
				{
					//int clientID;
					//newPacket >> clientID;

					//Vector2f pPos = players[clientID].getPos();

					//int TileID = tileMap.GetTileByPos(pPos).IdMapCord;

					//CreateEnemy(TileID);
					startRespEnemy = !startRespEnemy;
					//cout << "Enemy resp re: " << startRespEnemy << endl;
				}
				break;
				...
				break;
				case PacketTypes::UDPPort:
				{
					unsigned short port;
					newPacket >> port;
					players[i].UDPPort = port;
					cout << "Gracz: " << i << " udp port: " << port << endl;
					Packet p;
					p << PacketTypes::SetPlayerPosition << 1;
					sendPacketsUDP(&p, i, SendPacketsType::All);
				}
				break;
				default:
					break;
...
void Server::sendPacketsUDP(Packet* p, int clientID, SendPacketsType type)
{
	if (type == SendPacketsType::All) // send to all clients
	{
		...
		for (int i = 0; i < MAX_PLAYERS; i++)
		{
			if (clients[i] != nullptr)
				...
				{
					cout << "send" << endl;
					udpSocket.send(*p, clients[i]->getRemoteAddress().getLocalAddress(), players[i].UDPPort);
				}
		}
	}
	else if (type == SendPacketsType::WithoutOne) //send to all clients without this one
...
void Server::createBullet(Vector2f pos, Vector2f dirr, int ownerID, int entityID)
{
	Bullet* b = new Bullet();
	b->SetDirection(dirr);
	b->SetPos(pos);
		...
		if (p.getOnline())
		{
			if (p.weapon.shooting)
			{
				...
				if (p.pointsOfLaserEndBots.size() || p.pointsOfLaserEndWalls.size())
				{
					...
					//Packet pack;
					//pack << PacketTypes::SetLaserPointPlayer << p.getID() << p.laserPoint.x << p.laserPoint.y;
					//sendPackets(&pack, NULL, SendPacketsType::All);
...
Server::Server()
{
	...
	{
		cout << "Podaj inny port niz: " << serverPort << endl;
		cout << "Nowy port: "; cin >> serverPort;
	}
	while (udpSocket.bind(serverPortUDP) != Socket::Done)
	{
		cout << "Podaj inny port niz: " << serverPortUDP << endl;
		cout << "Nowy port: "; cin >> serverPortUDP;
	}
	system("cls");
	...
	cout << "Server TCP: " << IpAddress::getLocalAddress() << ":" << serverPort << endl;
	cout << "Server UDP: " << IpAddress::getLocalAddress() << ":" << serverPortUDP << endl;
